...
def bayes_risk_discrete(discrete_A, discrete_B, W, q):
    ...
    PxA, PxB = discrete_A['Prob'], discrete_B['Prob']
    R = 0.0
...
def find_strategy_discrete(discrete_A, discrete_B, W):
    ...
    for i in range(len(R_decide_A)):
        if R_decide_B[i] < R_decide_A[i]:
            q[i] = 1
        else:
            q[i] = 0
    # q = (R_decide_B < R_decide_A).astype(int)             -> found this with chatGPT as well, good to remember
...
def classify_discrete(measurements, q):
    ...
    """
    # Convert measurements (e.g., from [-10,10]) into indices [0,20]
    indices = measurements.astype(int) + 10

    indices[indices < 0] = 0
    indices[indices >= len(q)] = len(q) - 1
    labels = q[indices]
...
def visualize_discrete(discrete_A, discrete_B, q):
    ...
    plt.legend((bar_plot_A, bar_plot_B), (r'$p_{XK}(x,A)$', r'$p_{XK}(x,C)$'))
...
def visualize_2norm(cont_A, cont_B, q):
    ...
    plt.plot(xs, B_joint, c=colors[1], label='C')
